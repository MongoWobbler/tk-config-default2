...
class SubstancePainterStartVersionControlPlugin(HookBaseClass):
    ...
    def validate(self, settings, item):
        ...
        if not path:
            ...
            self.logger.error(error_msg, extra=self._get_save_as_action())
    ...
    def publish(self, settings, item):
        ...
        path = sgtk.util.ShotgunPath.normalize(self._session_path())
        ...
        self._save_session(version_path)
        self.logger.info(
...
class BlenderStartVersionControlPlugin(HookBaseClass):
    """
    Simple plugin to insert a version number into the blender file path if one
    does not exist.
    """
    ...
    def accept(self, settings, item):
        ...
        if path:
            ...
            if version_number is not None:
                self.logger.info(
                    "Blender '%s' plugin rejected the current session..." % (self.name,)
                )
                self.logger.info("  There is already a version number in the file...")
                self.logger.info("  Blender file path: %s" % (path,))
                return {"accepted": False}
        else:
            # the session has not been saved before (no path determined).
            # provide a save button. the session will need to be saved before
            # validation will succeed.
            self.logger.warn(
                "The Blender session has not been saved.", extra=self._get_save_as_action()
            )

        self.logger.info(
            "Blender '%s' plugin accepted the current session." % (self.name,),
            extra=self._get_version_docs_action(),
            )
    ...
    def publish(self, settings, item):
        ...
        # save to the new version path
        self._save_session(version_path)
        self.logger.info("A version number has been added to the Blender file...")
        self.logger.info("  Blender file path: %s" % (version_path,))
